...
2. **대시보드 (dashboard.html)**
   ...
   - "수강하기" 등 클릭 → `course-player.html?courseId=1&lessonId=1` 등 (lessonId는 해당 강의의 첫 레슨 ID 사용)

3. **수강 재생 (course-player.html)**
   - URL `courseId`, `lessonId`로 강의·레슨 로드 (lessonId 없거나 유효하지 않으면 첫 레슨으로 자동 이동)
   - 강의 ID에 맞는 영상/컨텐츠 재생
   - (구현한 경우) 챕터 목록·진도 저장
...
| 수강 재생   | courseId·lessonId로 강의·레슨 로드(미지정 시 첫 레슨), 강의 재생, (선택) 진도 저장 |
